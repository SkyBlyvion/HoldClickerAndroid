...
public final class HoldConfig {
    ...
    private static final long MAX_DURATION_MS = GestureDescription.getMaxGestureDuration();

    private HoldConfig() {}

    public static long getHoldDurationMs(Context context) {
        long savedValue = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                .getLong(HOLD_DURATION_MS, DEFAULT_DURATION_MS);
        return clampDurationMs(savedValue);
    }

    public static long setHoldDurationMs(Context context, long durationMs) {
        long safeValue = clampDurationMs(durationMs);
